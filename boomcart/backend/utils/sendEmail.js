...
const transporter = nodemailer.createTransport({
  ...
  port: 465, // Force 465 (SMTPS) which is more reliable on Render/cloud than 587
  secure: true, 
  auth: { user: process.env.EMAIL_USER, pass: process.env.EMAIL_PASS },
});

const sendEmail = async ({ to, subject, html }) => {
  const fromEmail = process.env.EMAIL_FROM || `Boomcart <${process.env.EMAIL_USER}>`;
...
<body><div class="wrap">
<div class="hdr"><h1>🛍️ Order Confirmed — Boomcart!</h1></div>
<div class="body">
...
<div class="ftr">© ${new Date().getFullYear()} Boomcart. All rights reserved.</div>
</div></body></html>`;
